...
(function () {
  ...
  function init() {
    ...
    // link; both import data/places/{id}.xml on init.
  ...
  function loadViewFromQuery() {
    ...
    window.BA.authority.load("place").then(function (recs) {
      ...
      fetch(rec.file)
        ...
        .then(function (text) {
          F._viewMode = true;
          importPlaceXML(text, (rec.file || "").split("/").pop());
          F.enterViewMode(document.getElementById("placeForm"), {
            type: "place",
            id: id,
            editorHref: "place-editor.html?load=" + id,
            collectionHref: "collection-places.html"
          });
  ...
  function buildRevisionHistory() {
    ...
    var accordion = document.getElementById("placeAccordion");
    if (accordion && accordion.parentNode) accordion.parentNode.insertBefore(section, accordion);
    else document.getElementById("placeForm").appendChild(section);
  }
